...
| **[OpenRouter](https://openrouter.ai/)** | OpenRouter | 無料モデルあり (レート制限付き) | **従量課金** | API経由で各種ツールと連携 | **マルチモデルゲートウェイ**。400+モデルに単一APIでアクセス。 |
...
### 7. [OpenRouter](https://openrouter.ai/) - 複数モデルを使い分けたいなら

- **IDE連携:** OpenRouter自体はIDEプラグインを提供していませんが、OpenAI互換のAPIを提供しているため、OpenAI APIに対応した各種ツールやエディタ拡張から利用可能です。
- **料金/無料枠:** 従量課金制で、各モデルの料金はプロバイダーの価格そのまま（マークアップなし）。一部のモデルには`:free`バリアントがあり、レート制限付きで無料利用可能。BYOK（Bring Your Own Key）も対応しており、月100万リクエストまで無料。
- **所感:**
    - Claude、GPT、Gemini、Llama等、400以上のモデルに単一のAPIでアクセスできるのが最大の魅力。
    - モデルごとの得意分野に応じて使い分けたい開発者や、複数プロバイダーを一元管理したいチームに向いています。
    - `:nitro`（高速応答優先）や`:floor`（低コスト優先）などのルーティングオプションも便利。
...
4.  複数モデルを柔軟に使い分けたいなら: `OpenRouter`で一元管理する。
